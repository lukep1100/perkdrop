...
(() => {
  ...
  function bind() {
    ...
    $('#listing-type')?.addEventListener('change',e=>{const claim=e.target.value==='claim',form=$('#merchant-form');$$('[data-claim-field]').forEach(x=>x.hidden=!claim);form.elements.capacity_total.required=claim;form.elements.redemption_verifier.required=claim;form.elements.booking_url.required=!claim;if(e.target.value==='event')form.elements.vertical.value='events';form.elements.fulfilment_mode.value=claim?'direct_claim':e.target.value==='event'?'information_only':'external_booking';});
