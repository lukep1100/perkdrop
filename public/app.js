...
(() => {
  ...
  function popup(d) {
    const t = type(d);
    return `<div class="popup"><img loading="lazy" src="${esc(image(d))}" alt="${image(d)===PLACEHOLDER?'Photo unavailable':esc(d.imageAlt||d.title)}"><span class="type-${t[0]}">${t[1]} ${t[2]}</span><h3>${esc(d.title)}</h3><b>${esc(d.merchant)}</b><p>${esc(d.timing || "")}</p><a data-internal href="${esc(route(d))}">View Drop</a> · <a target="_blank" href="${esc(navUrl(d))}">Navigate</a></div>`;
  }
  function pin(d) {
