...
export async function marketplacePublic(action,body={}){
  const response=await fetch(MARKETPLACE_API,{method:'POST',cache:'no-store',headers:{'content-type':'application/json'},body:JSON.stringify({action,...body})});
  const data=await response.json();
  if(!response.ok)throw new Error(String(data.error||'Request failed').replaceAll('_',' '));
  return data;
}
