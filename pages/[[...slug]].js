...
function PerkDropShell({deal,canonicalPath}){
 ...
 return h(React.Fragment,null,
  ...
  h("script",{src:"/app.js?v=v23-operations",defer:true})
 );
}
...
export async function getServerSideProps({params,resolvedUrl}){
 ...
 if(!Array.isArray(parts)||parts[0]!=="deals"||!parts[1])return {props:{deal:null,canonicalPath}};
 try{
  const response=await fetch(CATALOGUE_API,{headers:{accept:"application/json"}});
  if(!response.ok)return {props:{deal:null,canonicalPath}};
  ...
  return {props:{canonicalPath,deal:{slug:match.slug||requested,title:match.title||match.offerTitle||"Drop",merchant:match.merchant||match.businessName||"Local venue",description:match.description||match.conditions||"",image:match.image||match.imageUrl||match.mediaUrl||""}}};
