...
function PerkDropShell({deal,canonicalPath,isPreview=false}){
 ...
 const canonical=deal?`https://perkdrop.au/deals/${encodeURIComponent(deal.slug)}`:`https://perkdrop.au${canonicalPath||"/"}`;
 return h(React.Fragment,null,
  h(Head,null,
   h("meta",{charSet:"utf-8"}),h("meta",{name:"viewport",content:"width=device-width,initial-scale=1,viewport-fit=cover"}),h("meta",{name:"theme-color",content:"#08090e"}),h("meta",{name:"description",content:description}),isPreview?h("meta",{name:"robots",content:"noindex,nofollow"}):null,h("link",{rel:"canonical",href:canonical}),
  ...
  h("script",{src:"/app.js?v=v24-live-priority",defer:true})
 );
}
...
export async function getServerSideProps({params,resolvedUrl,res}){
 ...
 if(isPreview)res.setHeader('X-Robots-Tag','noindex, nofollow');
 const parts=params&&params.slug;
 const canonicalPath=(resolvedUrl||"/").split("?")[0]||"/";
 if(!Array.isArray(parts)||parts[0]!=="deals"||!parts[1])return {props:{deal:null,canonicalPath,isPreview}};
 try{
  const response=await fetch(CATALOGUE_API,{headers:{accept:"application/json"}});
  if(!response.ok)return {props:{deal:null,canonicalPath,isPreview}};
  ...
  return {props:{canonicalPath,isPreview,deal:{slug:match.slug||requested,title:match.title||match.offerTitle||"Drop",merchant:match.merchant||match.businessName||"Local venue",description:match.description||match.conditions||"",image:match.image||match.imageUrl||match.mediaUrl||""}}};
 }catch{return {props:{deal:null,canonicalPath,isPreview}};
}
}
