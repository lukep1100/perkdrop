...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const promo_code=clean(body.promo_code,100),conditions=clean(body.conditions,1800)||'Check with the business for final conditions.',booking_url=httpsUrl(clean(body.booking_url,1200)),media_url=httpsUrl(clean(body.media_url,1200));
    ...
    if(!business_name||!contact_name||!emailOk(contact_email)||!offer_title||!description||!location) return json({ok:false,error:"Please complete the required fields."},400);
    if(!authority_confirmed||!accuracy_confirmed) return json({ok:false,error:"Authority and accuracy confirmations are required."},400);
    ...
    const starts=date(body.starts_at),ends=date(body.ends_at),normal=num(body.normal_price),deal=num(body.deal_price);
    let discountType='custom';let discountPercent:number|null=null;if(normal!==null&&deal!==null&&normal>0&&deal<=normal){discountType='deal_price';discountPercent=Math.round(((normal-deal)/normal)*10000)/100}
    const {data:offer,error:offerError}=await supabase.from('merchant_offers').insert({merchant_id:merchant.id,title:offer_title,description,category:category||null,discount_type:discountType,discount_percent:discountPercent,normal_price:normal,deal_price:deal,promo_code:promo_code||null,conditions,starts_at:starts,ends_at:ends,location,city:city.toLowerCase().replace(/\s+/g,'-'),state,booking_url:booking_url||null,media_url:media_url||null,exclusive:exclusive_requested,status:'pending',metadata:{submission_source:'public_business_form'}}).select('id').single();
