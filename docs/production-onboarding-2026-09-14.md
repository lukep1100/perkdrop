...
- Public directory dependency follow-up: `perkdrop-business-directory` now uses the same Supabase JS retry baseline (version 2.102.0) and returns sanitized correlation/failure headers for merchants, offers or catalogue query failures. Edge version 10 is active; directory, market-filter and Union slug requests each returned HTTP 200 with distinct request IDs. Current read-only reconciliation is 308 active permanent merchants, 45 active catalogue rows expanding to 53 public deal entries, and 20 active locations; hero-rights states are 3 licensed, 1 merchant-authorised, 57 candidate and 247 missing. No imagery was promoted by this change.
