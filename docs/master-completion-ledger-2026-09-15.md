...
- Supabase unified logs currently show two recent Postgres SQL-editor audit-query errors (`42601` syntax error and `42703` missing `au.role`), but no API Gateway or Edge Function 5xx rows in the selected window; the historical catalogue incident remains insufficiently correlated for a definitive root cause.
